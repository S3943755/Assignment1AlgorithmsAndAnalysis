...
class CSRSpreadsheet(BaseSpreadsheet):
    ...
    def __init__(self):
        ...
        self.valA = []
        self.sumA = [0]
        self.colLength = -1 # is the max column length
        ...
        while rowFound != True:
            if focusRowIndex == rowIndex: # find desired row, if so update all future values to fix the new / changed value
                ...
                if (self.sumA[indexSumA] != self.sumA[indexSumA - 1]): # checks if the current row has an existing value stored
                    ...
                    # check if theres no values
                    if (difference == self.valA[colAIndex]):
                        ...
                        # loop for all columns (until appended), then if colA value is < colIndex prepend the data, if colAValue == colindex  replace, if colAValue > colIndex then look at next column BUT if sumOfCurrentRow == difference then append it as the colIndex is larger than the last column
                        j = colAIndex
                        sumOfCurrentRow = 0
                        storedNew = False

                        while(sumOfCurrentRow != difference):
                            sumOfCurrentRow += self.valA[j]
                            sumOfCurrentRow = round(sumOfCurrentRow, 1)

                            if (colIndex < self.colA[j]): # if input col value is before what is focused
                                ...
                                for i in range(indexSumA + 1, self.rowNum() + 1):
                                    self.sumA[i] = self.sumA[i] + value
                                sumOfCurrentRow = difference # ending loop as value already place
                                storedNew = True

                            elif (colIndex == self.colA[j]):
                                prevVal = self.valA[j]

                                self.colA[j] = colIndex
                                self.valA[j] = value
                                self.sumA[indexSumA] = self.sumA[indexSumA] + value - prevVal
                            ...
                            elif (storedNew == False and sumOfCurrentRow == difference and colIndex > self.colA[j]): # if input col value is after what is currently stored AND it is the final colIndex
                                j += 1
                                self.colA.insert(j, colIndex)
                                self.valA.insert(j, value)
                                self.sumA[indexSumA] = self.sumA[indexSumA] + value
                ...
                if (self.sumA[indexSumA] != self.sumA[indexSumA - 1]): # checks if the current row has an existing value stored
                    if (self.sumA[indexSumA] == self.valA[colAIndex]):
                        colAIndex += 1
        ...
        while (count != len(returnCells)):
            if (self.sumA[i - 1] != self.sumA[i]): # finding when a row has value by seeing if prev row is the same as teh current row we're focusing on
                ...
                if (difference == self.valA[colAIndex]): # only 1 value in the row
                    if (self.valA[colAIndex] == value): # check if the value is the target
                        returnCells.append([currRowIndex, self.colA[colAIndex]])

                    else: # if value isnt target
                    ...
                    while(sumOfCurrentRow != difference):
                        ...
                        if (self.valA[j] == value):
                            returnCells.append([currRowIndex, self.colA[j]])
            ...
            if (self.sumA[i - 1] != self.sumA[i]): # finding when a row has value by seeing if prev row is the same as teh current row we're focusing on
                ...
                if (difference == self.valA[colAIndex]): # meaning only 1 number
                    ...
                    while (sumOfCurrentRow != difference):
                        sumOfCurrentRow += self.valA[j]
                        sumOfCurrentRow = round(sumOfCurrentRow, 1)
                        currCell = Cell(row, self.colA[j], self.valA[j])
                        entriesList.append(currCell)
    ...
    def quickSortCol(self, lCells: [Cell]) -> [Cell]: # FIX ME sort for numbers in the same row w/diff columns
        """
        Sorts the list by row then column
        ...
        if len(lCells) <= 1:
            ...
            for i in range(1, len(lCells)):
                ...
                elif lCells[i].row == pivot.row: # if the rows are the same then check where append the values based on it's column number
